...
enum vtaskstate{
  ...
  VTASK_POSE_CALCULATION,
  VTASK_IK_CALCULATION,
  VTASK_POLY_GENERATE,
  VTASK_POLY_DRAW,
};
...
public:
  ...
  void calc_pose(int pn){
    fvector3 p;
    quaternion r;
    float t=animation_time;
    for(int i=pn;i<num;i+=PROCESSNUM){
      float ratio;
  ...
  void setpose(float t,int pn){
    ...
    for(int s=0;s<PROCESSNUM-1;s++)clientps[s].vtaskstate = VTASK_POSE_CALCULATION;
    calc_pose(0);
    barrier_sync();
  }
  ...
  void calcall(Matrix4 root){
    ...
    // if((getMicrotime()-begin)/1000000.f*30.f>100){
    //   return;
    // }
    ...
    animation_time=(getMicrotime()-begin)/1000000.f*30.f;
    setpose(animation_time,0);
    ...
    for(int s=0;s<PROCESSNUM-1;s++)clientps[s].vtaskstate = VTASK_IK_CALCULATION;
    updateiks(0);
    barrier_sync();
#ifdef VISUALDEBUG
...
#define REDUCE_IKLOOP
  ...
  void updateiks(int pn){
    //  if(pn!=1)return;
    for(int i=pn;i<// 1||false&&
	  (int)(sizeof(iks)/sizeof(iks[0]));i+=PROCESSNUM){
      update1ik(&iks[i]);
    }
  }
};
